...
def kdtree_recursive_build(root, db, point_indices, axis, leaf_size):
    ...
    if len(point_indices) > leaf_size:
        ...
        root.value = (left_leaf_value + right_leaf_value) / 2
        root.left = kdtree_recursive_build(root.left, db, point_indices_sorted[:median_right_leaf_index], axis_round_robin(axis, dim=db.shape[1]), leaf_size)
